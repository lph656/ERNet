...
class ResizeShortestEdge(Augmentation):
    ...
    @torch.jit.unused
    def __init__(
        self, short_edge_length, max_size=sys.maxsize, sample_style="range", is_train=True, interp=Image.BILINEAR
    ):
        """
        Args:
    ...
    def get_transform(self, image):
        ...
        if self.is_train:
            interpolation_methods = [cv2.INTER_CUBIC, cv2.INTER_LINEAR, cv2.INTER_NEAREST, cv2.INTER_AREA,
                                     cv2.INTER_LANCZOS4, cv2.INTER_LINEAR_EXACT]
            interpolation_method = random.choice(interpolation_methods)
            self.interp = interpolation_method
        else:
